...
export async function seedOrders(
  ...
  for (const user of users) {
    ...
    const seller =
      faker.helpers.arrayElement(users.filter((u) => u.id !== user.id)) ||
      users[0];

    // 2. On génère entre 1 et 3 articles pour cette commande
    const selectedArticles = faker.helpers.arrayElements(articles, {
      min: 1,
      max: 3,
    });
    ...
    const articleDetails = selectedArticles.map((art) => ({
      articleId: art.id, // Ici on passe l'entité Article complète comme demandé par ton type
  ...
  console.log(
    `✅ Seed terminé : Des commandes ont été créées pour ${users.length} acheteurs.`,
  );
}
